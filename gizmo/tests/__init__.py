...
class InclusionTagsTestCase(unittest.TestCase):
    ...
    def test_get_gizmos(self):
        ...
        # if a conf is provided that contains gizmos for the provided slot, return a list of gizmos 
        # matching the slot and current url pattern name. Gizmos are only filtered by url name if a 
        # url name is provided in the gizmo.
        settings.ROOT_GIZMOCONF = 'gizmo.tests.gizmos'
        gizmos = node.get_gizmos('slot_name', request)
        ...
        self.failUnlessEqual(gizmos, [('gizmo_test_tags', 'test_simple_tag', 'another_slot_name', ['url_name'])])
